...
from app.forms import HeyYouForm, AgeInForm, OrderTotalForm
...
def age_in(request):
    ...
    if form.is_valid():
        ...
        return render(request, "age_in.html", {"form": form, "age": age, "year": year, "BYear": BYear})
    else:
        form = AgeInForm()
...
def order_total(request):
    ...
    if form.is_valid():
        ...
        return render(request, "order_total.html", {"form": form, "total": total, "burgers": burgers, "fries": fries, "drinks": drinks})
    else:
        form = OrderTotalForm()
